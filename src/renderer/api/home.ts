...
// 获取新碟上架（按地区）: area 可选值参考 Netease 接口（ALL, ZH, EA, KR, JP）
export const getNewAlbumByArea = (params: { area: string; limit?: number; type?: 'new' | 'hot' }) => {
  const { area, limit = 20, type } = params;
  const query: Record<string, any> = { area, limit };
  if (type) query.type = type;
  return request.get<IAlbumNew>('/album/new', { params: query });
};
