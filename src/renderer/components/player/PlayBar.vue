<template>
  <div
    class="music-play-bar glass shadow-soft"
    :class="[
    ...
    <div class="play-bar-img-wrapper" @click="setMusicFull">
      ...
      <div class="hover-arrow">
        <div class="hover-content">
          ...
          <component :is="arrowIcon" class="text-3xl" />
    ...
    <div class="audio-button">
      <div class="audio-volume custom-slider" @wheel.prevent="handleVolumeWheel">
        <div class="volume-icon" @click="mute">
          <component :is="getVolumeIcon" class="text-xl" />
        </div>
      ...
      <n-tooltip v-if="!isMobile" trigger="hover" :z-index="9999999">
        <template #trigger>
          <component :is="playModeIcon" class="text-xl cursor-pointer" @click="togglePlayMode" />
        </template>
        {{ playModeText }}
      </n-tooltip>
      <n-tooltip v-if="!isMobile" trigger="hover" :z-index="9999999">
...
const getVolumeIcon = computed(() => {
  // 0 静音 icon-tabler-volume-off 0.5 icon-tabler-volume-2 1 icon-tabler-volume
  if (playerStore.volume === 0) {
    return 'icon-tabler-volume-off';
  }
  if (playerStore.volume <= 0.5) {
    return 'icon-tabler-volume-2';
  }
  return 'icon-tabler-volume';
});
...
// 箭头切换图标
const arrowIcon = computed(() => (musicFullVisible.value ? 'icon-tabler-chevron-down' : 'icon-tabler-chevron-up'));
// 播放模式
const playMode = computed(() => playerStore.playMode);
const playModeIcon = computed(() => {
  switch (playMode.value) {
    case 0:
      return 'icon-tabler-repeat';
    case 1:
      return 'icon-tabler-repeat-once';
    case 2:
      return 'icon-tabler-shuffle';
    default:
      return 'icon-tabler-repeat';
  }
});
...
.music-play-bar {
  @apply h-20 w-full absolute bottom-0 left-0 flex items-center box-border px-6 py-2 pt-3 rounded-2xl;
  background: transparent; /* glass provides backdrop */
  z-index: 9999;
...
.music-buttons {
  ...
  .iconfont {
    @apply text-2xl transition-transform duration-150 ease-out;
  ...
  .icon {
    @apply text-3xl transition-transform duration-150 ease-out hover:text-green-500;
  }
  ...
  &-play {
    @apply flex justify-center items-center w-20 h-12 rounded-full mx-4 transition-all duration-150 text-gray-600 dark:text-gray-200;
    background: linear-gradient(135deg, rgba(165,180,252,.25), rgba(52,211,153,.25));
...
.audio-volume {
  ...
  .volume-slider {
    ...
    .dark & { background: rgba(17,17,17,.7); }
    @apply border border-gray-200 dark:border-gray-700;

    .volume-percentage {
      @apply absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-medium px-2 py-1 rounded-md;
      background: rgba(255,255,255,.9);
      .dark & { background: rgba(17,17,17,.9); }
      @apply border border-gray-200 dark:border-gray-700;
