  ...
  <div class="recommend-singer">
    <div class="recommend-singer-list">
      ...
      >
        ...
        >
          <!-- 优先显示每日推荐；无数据时回退随机热门 -->
          <div v-if="getDisplayDaySongs.length" class="recommend-singer-item relative">
            <div
              :style="setBackgroundImg(dailyCoverUrl)"
            ...
            >
              <div class="font-bold text-lg">每日推荐</div>
              <div class="mt-2">
          ...
          <div v-else-if="randomHot && randomHot.tracks?.length" class="recommend-singer-item relative">
            <div :style="setBackgroundImg(getImgUrl(randomHot.coverUrl, coverSize))" class="recommend-singer-item-bg"></div>
            <div class="recommend-singer-item-count p-2 text-base text-gray-200 z-10 cursor-pointer" @click="showRandomHot">
              <div class="font-bold text-lg">随机热门</div>
              <div class="mt-2">
                <p v-for="item in randomHot.tracks.slice(0, 5)" :key="item.id" class="text-el">{{ item.name }}<br /></p>
              </div>
            </div>
          </div>
        </n-carousel-item>
...
import { getListDetail, getRecommendList } from '@/api/list';
...
// 随机热门推荐（替代固定“每日推荐”）
const randomHot = ref<{ playlistId: number; name: string; coverUrl: string; tracks: SongResult[] } | null>(null);
const loadRandomHot = async () => {
  try {
    // 拉取推荐歌单，随机挑选一个
    const { data } = await getRecommendList(60);
    const lists = data.result || data.playlists || [];
    if (!lists.length) return;
    const pick = lists[Math.floor(Math.random() * lists.length)];
    const { data: detail } = await getListDetail(pick.id);
    const tracks = (detail.playlist?.tracks || []).map((track: any) => ({
      ...track,
      source: 'netease',
...
const loadNonUserData = async () => {
  try {
    ...
    // 获取热门歌手（随机偏移并打散，保证每次进入有变化）
    const offset = Math.floor(Math.random() * 200);
    let { data: singerData } = await getHotSinger({ offset, limit: 100 });
    let artists = shuffleArray(singerData.artists || []).slice(0, 24);
    // 如果随机偏移没有拿到数据，回退到 offset=0
    if (!artists.length) {
      const resp = await getHotSinger({ offset: 0, limit: 100 });
      singerData = resp.data as any;
      artists = shuffleArray(singerData.artists || []).slice(0, 24);
    }
    hotSingerData.value = { ...singerData, artists } as IHotSinger;
  } catch (error) {
...
// 每日推荐封面：优先取第一首歌曲的专辑封面，其次取歌曲自身 picUrl，最后回退随机热门封面，避免黑屏
const coverSize = computed(() => (isMobile.value ? '300y300' : '500y500'));
const dailyCoverUrl = computed(() => {
  const songs = getDisplayDaySongs.value || [];
  if (songs.length > 0) {
    const s: any = songs[0];
    if (s?.al?.picUrl) return getImgUrl(s.al.picUrl, coverSize.value);
    if (s?.picUrl) return getImgUrl(s.picUrl, coverSize.value);
  }
  return randomHot.value?.coverUrl ? getImgUrl(randomHot.value.coverUrl, coverSize.value) : '';
});
const showRandomHot = () => {
  if (!randomHot.value?.tracks?.length) return;
  navigateToMusicList(router, {
    id: randomHot.value.playlistId,
    type: 'playlist',
    name: randomHot.value.name,
    songList: randomHot.value.tracks,
    canRemove: false
  });
};
...
.recommend-singer {
  ...
  &-item {
    ...
    /* 悬停时增强动效：背景轻微放大与略微变暗，使信息层更突出 */
    &:hover .recommend-singer-item-bg {
...
/* Carousel custom arrows - ensure always visible and red */
.custom-arrows {
  position: absolute;
  inset: 0;
  display: flex;
  ...
  align-items: center;
  padding: 0 8px;
  pointer-events: none; /* allow inner buttons to capture */
  z-index: 100;
}
.arrow-btn {
  pointer-events: auto;
  width: 40px;
  height: 40px;
  border: 1px solid rgba(255, 59, 48, 0.6); /* red with opacity */
  border-radius: 9999px;
  background: rgba(255, 255, 255, 0.5); /* semi-transparent so text behind is readable */
...
.arrow-btn:hover { background: #ffffff; border-color: #ff3b30; transform: scale(1.05); }
.arrow-btn.left { margin-right: auto; }
.arrow-btn.right { margin-left: auto; }

/* Mobile/touch: hide custom arrows, rely on swipe */
@media (hover: none) {
