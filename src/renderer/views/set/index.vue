<template>
  <div class="settings-container">
    ...
    <n-scrollbar ref="scrollbarRef" class="settings-content" @scroll="handleScroll">
      <div class="set-page">
        <div class="page-header">{{ t('settings.title') || '设置' }}</div>
        <!-- 基础设置 -->
        ...
        <!-- 插件（音源）设置 -->
        <div id="plugins" ref="pluginsRef" class="settings-section">
          <div class="settings-section-title">音源插件</div>
          <div class="settings-section-content">
            <div class="set-item">
              <div>
                <div class="set-item-title">远程插件列表</div>
                <div class="set-item-content">来自 Cloudflare 代理的安全清单（可开启/关闭）</div>
              </div>
              <n-button size="small" :loading="pluginsLoading" @click="loadPlugins">刷新</n-button>
            </div>

            <div class="ui-card mt-2" v-if="pluginsRemote.length">
              <div class="flex flex-col gap-2">
                <div v-for="p in pluginsRemote" :key="p.url" class="flex items-center justify-between">
                  <div>
                    <div class="font-medium">{{ p.name }} <span class="text-xs text-gray-500" v-if="p.version">v{{ p.version }}</span></div>
                    <div class="text-xs text-gray-400 break-all">{{ p.url }}</div>
                  </div>
                  <n-switch v-model:value="enabledMap[p.url]" @update:value="(v)=>togglePlugin(p, v)"/>
                </div>
              </div>
            </div>

            <div class="set-item mt-3">
              <div>
                <div class="set-item-title">自定义插件</div>
                <div class="set-item-content">添加额外插件 URL（将通过 Cloudflare 代理加载）</div>
              </div>
              <div class="flex gap-2">
                <n-input v-model:value="customUrl" placeholder="输入插件 JS URL" style="width: 420px"/>
                <n-input v-model:value="customName" placeholder="显示名称" style="width: 180px"/>
                <n-button size="small" type="primary" :disabled="!customUrl" @click="addCustomPlugin">添加</n-button>
              </div>
            </div>

            <div v-if="customPlugins.length" class="ui-card mt-2">
              <div class="font-medium mb-2">已添加的自定义插件</div>
              <div class="flex flex-col gap-2">
                <div v-for="c in customPlugins" :key="c.url" class="flex items-center justify-between">
                  <div>
                    <div class="font-medium">{{ c.name || 'Custom' }}</div>
                    <div class="text-xs text-gray-400 break-all">{{ c.url }}</div>
                  </div>
                  <div class="flex items-center gap-2">
                    <n-switch v-model:value="enabledMap[c.url]" @update:value="(v)=>togglePlugin(c, v)"/>
                    <n-button size="tiny" type="error" quaternary @click="removeCustomPlugin(c.url)">删除</n-button>
                  </div>
                </div>
              </div>
            </div>
...
onMounted(async () => {
  ...
  // 初始化插件数据默认值
  if (!Array.isArray(setData.value.enabledPlugins)) {
    setData.value = { ...setData.value, enabledPlugins: [] };
  }
  if (!Array.isArray(setData.value.customPlugins)) {
    setData.value = { ...setData.value, customPlugins: [] };
  }
  await loadPlugins();
});
...
// ====== 插件（音源）逻辑 ======
const pluginsRef = ref<HTMLElement | null>(null);
const pluginsLoading = ref(false);
const pluginsRemote = ref<{ name: string; url: string; version?: string }[]>([]);
const customUrl = ref('');
const customName = ref('');
const enabledMap = ref<Record<string, boolean>>({});

const customPlugins = computed(() => (setData.value.customPlugins || []) as {name:string;url:string}[]);

const syncEnabledMapFromSettings = () => {
  const list: string[] = setData.value.enabledPlugins || [];
  const map: Record<string, boolean> = {};
  list.forEach((u) => (map[u] = true));
  // 保证 custom 插件也在 map 中
  (setData.value.customPlugins || []).forEach((c: any) => { if (map[c.url] === undefined) map[c.url] = true; });
  enabledMap.value = map;
};

const saveEnabledMapToSettings = () => {
  const list = Object.keys(enabledMap.value).filter((k) => enabledMap.value[k]);
  setData.value = { ...setData.value, enabledPlugins: list };
};

const loadPlugins = async () => {
  pluginsLoading.value = true;
  try {
    const data = await getRemotePlugins();
    pluginsRemote.value = Array.isArray(data?.plugins) ? data.plugins : [];
    syncEnabledMapFromSettings();

    // 若用户尚未启用任何插件，则按关键词默认启用一组常用源
    const currentEnabled = Array.isArray(setData.value.enabledPlugins)
      ? setData.value.enabledPlugins
      : [];
    if (currentEnabled.length === 0 && pluginsRemote.value.length > 0) {
      const preferKeywords = ['qq', '酷狗', 'kugou', '咪咕', 'migu', 'bilibili', 'spotify', '网易'];
      for (const p of pluginsRemote.value) {
        const name = (p.name || '').toLowerCase();
        if (preferKeywords.some((kw) => name.includes(kw))) {
          enabledMap.value[p.url] = true;
        }
      }
      saveEnabledMapToSettings();
    }
  } finally {
...
const togglePlugin = (p: {url:string; name?:string}, v: boolean) => {
  enabledMap.value[p.url] = v;
  saveEnabledMapToSettings();
};

const addCustomPlugin = () => {
  if (!customUrl.value) return;
  const arr = [...(setData.value.customPlugins || [])];
  const exist = arr.find((x: any) => x.url === customUrl.value);
  if (!exist) arr.push({ url: customUrl.value, name: customName.value || 'Custom' });
  setData.value = { ...setData.value, customPlugins: arr };
  enabledMap.value[customUrl.value] = true;
  saveEnabledMapToSettings();
  customUrl.value = '';
  customName.value = '';
};

const removeCustomPlugin = (url: string) => {
  const arr = (setData.value.customPlugins || []).filter((x: any) => x.url !== url);
  setData.value = { ...setData.value, customPlugins: arr };
  delete enabledMap.value[url];
  saveEnabledMapToSettings();
};
...
// 定义设置分类（使用 computed，随语言切换自动更新）
const settingSections = computed(() => [
  { id: 'basic', title: t('settings.sections.basic') },
  { id: 'playback', title: t('settings.sections.playback') },
  // i18n 若缺少该 key，则回退为中文默认文案
  { id: 'plugins', title: t('settings.sections.plugins') || '音源插件' },
...
const handleScroll = (e: any) => {
  ...
  const sections = [
    ...
    // { id: 'about', ref: aboutRef }, // 已隐藏
    { id: 'donation', ref: donationRef }
  ] as const;

  let lastValidSection = currentSection.value;
  for (const section of sections) {
    const el = section.ref?.value as HTMLElement | undefined;
    if (el && Number.isFinite(el.offsetTop)) {
      const { offsetTop } = el;
      if (scrollTop >= offsetTop - 100) lastValidSection = section.id;
    }
  }

  if (lastValidSection && lastValidSection !== currentSection.value) {
...
.settings-nav {
  @apply w-44 h-full flex-shrink-0 border-r border-gray-200 dark:border-gray-700;
...
.settings-section {
  ...
  &-title {
    @apply sticky top-0 z-10 text-base font-medium mb-4 px-1 py-2;
    @apply bg-white/70 dark:bg-dark/70 backdrop-blur supports-[backdrop-filter]:backdrop-blur;
    @apply text-gray-700 dark:text-gray-100 border-b border-gray-200/70 dark:border-gray-700/70;
  }
...
.set-item {
  @apply flex items-center justify-between p-4 rounded-xl transition-all;
  @apply bg-light dark:bg-dark text-gray-900 dark:text-white;
  @apply border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md;
