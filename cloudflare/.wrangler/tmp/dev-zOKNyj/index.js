...
// .wrangler/tmp/bundle-JP7PQ4/checked-fetch.js
...
var REDIRECT_URI = "https://your-worker-domain.com/auth/github/callback";
...
github.get("/auth/github/callback", async (c) => {
  ...
  const row = await c.env.DB.prepare("SELECT * FROM users WHERE github_id=?1").bind(ghUser.id.toString()).first();
  let userId = row?.id;
  if (!row) {
    const res = await c.env.DB.prepare(
      "INSERT INTO users (username, github_id) VALUES (?1, ?2)"
    ).bind(ghUser.login, ghUser.id.toString()).run();
    userId = res.meta.last_row_id;
  }
  const token = await sign2({ uid: userId, username: ghUser.login }, c.env.JWT_SECRET, {
    expiresIn: "2h"
  });
  return c.json({ token });
});
...
var app = new Hono2();
app.use("*", cors());
var createToken = /* @__PURE__ */ __name((payload, env) => sign2(payload, env.JWT_SECRET, { expiresIn: "2h" }), "createToken");
var authGuard = createMiddleware(async (c, next) => {
  const auth = c.req.header("Authorization");
  if (!auth) return c.json({ error: "Missing token" }, 401);
  try {
    const token = auth.replace("Bearer ", "");
    c.set("user", await verify2(token, c.env.JWT_SECRET));
    await next();
  } catch (_) {
...
app.post("/api/login", async (c) => {
  const { username, password } = await c.req.json();
  const row = await c.env.DB.prepare("SELECT * FROM users WHERE username=?1").bind(username).first();
...
// .wrangler/tmp/bundle-JP7PQ4/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-JP7PQ4/middleware-loader.entry.ts
